...
import { PREDEFINED_PERSONAS, QuickFillPersona } from '@/lib/personas';
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Real authentication against the backend (bcrypt-verified against the
    // seeded Postgres users) — this used to be a purely client-side check
    // against a hardcoded persona array, which is why every persona ended
    // up sharing the same data no matter who "logged in."
    ...
    if (res.success && res.data) {
      ...
      router.push(res.data.user.role === 'ADMIN' ? '/executive' : '/dashboard');
    } else {
      const message = res.error?.message || 'Invalid credentials. Please verify your CSTU email and password.';
  ...
  const handleSelectPreset = (persona: QuickFillPersona) => {
    setEmail(persona.email);
    setPassword(persona.demoPassword);
    setEmailError('');
    setPasswordError('');
    setAuthError('');
  };
      ...
      <div className="w-full max-w-md space-y-6 relative z-10">
        ...
        <Card className="glass-panel border-slate-700/60 shadow-2xl">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-semibold">Campus Sign In</CardTitle>
            <CardDescription>
              Sign in with your CSTU campus credentials to access your personalized dashboard.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            {/* Quick Persona Preset Selector — fills the form only; the actual
                login still goes through the backend against the seeded DB users. */}
            ...
            <div className="pt-2 text-center text-[11px] text-slate-500 flex items-center justify-center gap-1.5 font-mono">
              ...
              Credentials verified against the CampusConnect API
            </div>
          </CardContent>
        </Card>
      </div>
