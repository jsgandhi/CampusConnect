...
export default function AppointmentsPage() {
  ...
  const handleBookAppointment = async (e: React.FormEvent) => {
    ...
    if (!purpose.trim()) {
      setBookingError('Please describe the purpose of your session.');
      return;
    }
    if (purpose.trim().length < 5) {
      setBookingError('Purpose must be at least 5 characters long.');
      return;
    }
    ...
    if (res.success && res.data) {
      ...
      // Shown INSIDE the modal (not the page-level banner behind it, which
      // was invisible until the modal was closed — that's why it looked
      // like clicking Confirm "did nothing").
      setBookingError(res.error?.message || 'Booking failed. Please try again.');
    }
  };
    ...
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100">
      ...
      >
        <form onSubmit={handleBookAppointment} className="space-y-4">
          {bookingError && (
            <Alert variant="danger" title="Couldn't Schedule Appointment">
              {bookingError}
            </Alert>
          )}
